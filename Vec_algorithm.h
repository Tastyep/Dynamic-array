...
#define vec_elem_found(it) ((it) == -1 ? 0 : 1)

/*
** vec_find(vec, value [beg, end])
*/

#define vec_find_1(vec, value) vec_find_3(vec, value, 0, (vec)->size)

#define vec_find_3(vec, value, beg, end)                                       \
  ((vec)->buffer.buffer_value = value,                                         \
   vector_find((char *)vec_data(vec), vec_data_size(vec),                      \
               (vec)->buffer.value_bytes, beg, end))

#define distribute_vec_find(vec, _1, _2, _3, FUNC, ...) FUNC
#define vec_find(vec, ...)                                                     \
  distribute_vec_find(vec, __VA_ARGS__, vec_find_3, _NULL, vec_find_1,         \
...
#define vec_find_if(vec, ...)                                                  \
  distribute_vec_find_if(vec, __VA_ARGS__, vec_find_if_3, _NULL,               \
                         vec_find_if_1, _NULL)(vec, __VA_ARGS__)

/*
** vec_count(vec, value [beg, end])
*/

#define vec_count_1(vec, value) vec_count_3(vec, value, 0, vec_size(vec))

#define vec_count_3(vec, value, beg, end)                                      \
  ((vec)->buffer.buffer_value = value,                                         \
   vector_count((char *)vec_data(vec), vec_data_size(vec),                     \
                (vec)->buffer.value_bytes, beg, end))

#define distribute_vec_count(vec, _1, _2, _3, FUNC, ...) FUNC
#define vec_count(vec, ...)                                                    \
  distribute_vec_count(vec, __VA_ARGS__, vec_count_3, _NULL, vec_count_1,      \
                       _NULL)(vec, __VA_ARGS__)
...
#define vec_count_if_3(vec, cond_func, beg, end)                               \
  vector_count_if((char *)vec_data(vec), vec_data_size(vec), cond_func, beg,   \
                  end)

#define distribute_vec_count_if(vec, _1, _2, _3, FUNC, ...) FUNC
#define vec_count_if(vec, ...)                                                 \
  distribute_vec_count_if(vec, __VA_ARGS__, vec_count_if_3, _NULL,             \
                          vec_count_if_1, _NULL)(vec, __VA_ARGS__)
...
#define vec_for_each(vec, ...)                                                 \
  distribute_vec_for_each(vec, __VA_ARGS__, vec_for_each_3, _NULL,             \
                          vec_for_each_1, _NULL)(vec, __VA_ARGS__)
...
#define vec_replace(vec, ...)                                                  \
  distribute_vec_replace(vec, __VA_ARGS__, vec_replace_4, _NULL,               \
                         vec_replace_1, _NULL)(vec, __VA_ARGS__)
